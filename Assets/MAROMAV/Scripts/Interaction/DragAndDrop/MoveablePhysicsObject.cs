...
namespace MAROMAV.CoworkSpace
{
    ...
    public class MoveablePhysicsObject : BaseInteractiveObject, IPointerDownHandler
    {
        ...
        private Vector3 lookPos = new Vector3(-2, 1.2f, -5);
        ...
        void Awake()
        {
            ...
            photo = gameObject.GetComponent<Photo>();
            ...
            rotationOnTouch = isLogoModel ? true : false;
        }

        // Input state is checked in Update().
        protected override void OnDrag()
        {
            if (ControllerInteractionGoogleVR.controllerMode == ControllerInteractionGoogleVR.ControllerMode.HAND)
            {
        ...
        protected override void OnSelect()
        {
            if (ControllerInteractionGoogleVR.controllerMode != ControllerInteractionGoogleVR.ControllerMode.HAND)
            {
                Debug.Log("controllerMode is not hand on select return " + ControllerInteractionGoogleVR.controllerMode);
                return;
            }
            ...
            if (d >= minDistance && d <= maxDistance)
            {
                ...
                if (!isLogoModel)
                {
                    photo.IsCatched = true;
                    if (photo.AssignedColorForUserId.Equals(photonView.ownerId))
                    {
                        photo.OnChangeColor();
                    }
                }

                ControllerInteractionGoogleVR.Instance.painter.SetGrabMode();
        ...
        protected override void OnDeselect()
        {
            ...
            if (!isLogoModel)
            {
                photo.IsCatched = false;
                photo.OnChangeColorBasic();
            }
        ...
        // rigidbody rotate to look position.
