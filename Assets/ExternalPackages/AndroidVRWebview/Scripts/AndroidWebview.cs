...
namespace MAROMAV.CoworkSpace
{
    ...
    public class AndroidWebview : Photon.PunBehaviour
    {
        ...
        public ArrayList waitLine = new ArrayList();


        public void AddToWaitLine()
        {
            int playerId = (int)PhotonNetwork.player.CustomProperties["position"];
            photonView.RPC("AddMe", PhotonTargets.All, playerId);
        }

        public void RemoveFromWaitLine()
        {
            int playerId = (int)PhotonNetwork.player.CustomProperties["position"];
            photonView.RPC("RemoveMe", PhotonTargets.All, playerId);
        }
        ...
        void Update()
        {
            // Vector2 local = CalculatePositionFrom (GameObject.Find("Reticle").transform.position);
            // local= new Vector2(local.x,local.y);
#if !UNITY_EDITOR
        ...
        {
            if (ControllerInteractionGoogleVR.Instance.IsNowPointerInWebViewer)
            {
                ...
                Vector2 local = CalculatePositionFrom(ControllerInteractionGoogleVR.Instance.ReticlePosition);
                local = new Vector2(local.x, local.y);
                photonView.RPC("SendEvent", PhotonTargets.All, local, (int)actionEvent);
                //SendEvennt(local, actionEvent);
                isDown = false;
                isUp = false;
                isHover = false;
            }
        ...
        public void GoBack()
        {
            ...
            if ((int)waitLine[0] == playerId)
            {
                photonView.RPC("GoBackRPC", PhotonTargets.All);
            }
        }
    ...
    public void LoadUrlHome(String newUrl)
    {
            int playerId = (int)PhotonNetwork.player.CustomProperties["position"];
            if ((int)waitLine[0] == playerId)
            {
                photonView.RPC("LoadUrlHomeRPC", PhotonTargets.All, newUrl);
            }
    ...
    [PunRPC]
        void LoadUrlHomeRPC(String newUrl)
        {
            url = newUrl;
         ...
         */
        public Vector2 CalculatePositionFrom(Vector3 hit)
        {
            Vector3 position = transform.InverseTransformPoint(hit);
